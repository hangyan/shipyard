...
import (
	...
	"github.com/hangyan/citadel/cluster"
	"github.com/hangyan/citadel/scheduler"
	"github.com/hangyan/shipyard"
	"github.com/hangyan/shipyard/dockerhub"
)
...
func (m *Manager) init() []*shipyard.Engine {
	...
	for _, d := range engines {
		...
		if err := getEngineMetrics(d.Engine); err != nil {
			logger.Error("error getting metrics of engine: %s, err")
		}
		if _, err := r.Table(tblNameConfig).Replace(d).RunWrite(m.session); err != nil {
			logger.Error("error update metrics of engine: %s, err")
		}
		engs = append(engs, d.Engine)

		logger.Infof("loaded engine id=%s addr=%s with %d cpus and %.2f memory", d.Engine.ID, d.Engine.Addr, int(d.Engine.Cpus), d.Engine.Memory)
	}
...
func (m *Manager) Container(id string, all bool) (*citadel.Container, error) {
	containers := m.clusterManager.ListContainers(all)
...
func (m *Manager) Scale(container *citadel.Container, count int) error {
	...
	logger.Infof("origin instance count : %d", containerCount)
